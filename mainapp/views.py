...
from .models import Course,Enroll,course_outlines,Course_details,Quiz,Question,Answer,Completed,Quizcompleted,Grade
from rest_framework.viewsets import ModelViewSet,GenericViewSet
from .permitions import ISADMINORREADONLY,ISENrolled,ISMYcourse
from .serializers import CourseSerializer,EnrolSerializer,CourseOutlineSerializer,CoursedetailSerializer,QuizSerializer,QuestionSERializer,AnswerSerilizer,ProfileSerializer,IscompleteSerilizer,IsquizcompleteSerilizer,GradeSerializer
...
class QuestionsViewSet(ModelViewSet):
    permission_classes=[IsAuthenticated,ISADMINORREADONLY,ISENrolled]
    def get_queryset(self):
        return Question.objects.filter(quiz_id=self.kwargs['quize_pk'])
...
class CompleteViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return Completed.objects.filter(user=student)
   
    serializer_class=IscompleteSerilizer
    def get_serializer_context(self):
        return {
            'userid':self.request.user.id
        }
    
class CompletequizViewSet(ModelViewSet):
    permission_classes=[IsAuthenticated]
    def get_queryset(self):
        
        (student,created)=Profile.objects.get_or_create(user_id=self.request.user.id)
        print(self.request.user.id)
        return Quizcompleted.objects.filter(user=student)
   
    serializer_class=IsquizcompleteSerilizer
    def get_serializer_context(self):
        return {
            'userid':self.request.user.id
        }
class GradeViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        (student,created)=Profile.objects.get_or_create(user_id=self.request.user.id)
        print(self.request.user.id)
        return Grade.objects.filter(user=student)
