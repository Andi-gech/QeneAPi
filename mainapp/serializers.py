...
from .models import Course,Profile,Course_details,Enroll,course_outlines,Quiz,Question,Answer,Completed,Quizcompleted,Grade
...
class CourseSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model=Course
        fields=['id','Course_name','Teacher','description','course_image']
...
class AnswerSerilizer(serializers.ModelSerializer):
    ...
    def save(self, **kwargs):
       question=get_object_or_404(Question, id=self.context['question_id'])
       if question:
            self.validated_data['question'] = question
       return super().save(**kwargs)
    
class IscompleteSerilizer(serializers.ModelSerializer):
    user= serializers.PrimaryKeyRelatedField(read_only=True)
    class Meta:
        model=Completed
        fields=['id','courseoutlinee','user']
    def save(self, **kwargs):
        (student,created)=Profile.objects.get_or_create(user_id=self.context['userid'])
        if student:
            self.validated_data['user'] = student
        return super().save(**kwargs)
class IsquizcompleteSerilizer(serializers.ModelSerializer):
    user= serializers.PrimaryKeyRelatedField(read_only=True)
    class Meta:
        model=Quizcompleted
        fields=['id','quiz','user']
...
class GradeSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model=Grade
        fields=['id','courseoutlines','course','percent','user']
    def save(self, **kwargs):
